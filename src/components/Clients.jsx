...
import extra   from '../assetcompany/8.png';
import img6    from '../assetcompany/6.png';
import img7    from '../assetcompany/7.png';
        ...
        {repeated.map((src, i) => (
          <div key={i} className="m-logo-wrap" data-hover>
            <img
              src={src}
              alt={`client-${i}`}
              className={`m-logo-img ${src === nextias || src === bttpl ? 'm-logo-large' : ''} ${src === img6 ? 'm-logo-livesmart' : ''} ${src === bttpl ? 'm-logo-bttpl' : ''}`}
            />
          </div>
        ))}
      </div>
...
export default function Clients() {
  return (
    <section className="clients-section">
      ...
      <p className="clients-subtitle">Brands that trust Socio Maverick</p>
